...
\begin{tabular}{|ll|}
\hline
\textbf{Año:} IV  & \textbf{Requisitos:} MA-0615 Ecuaciones Diferenciales, MA-0541 Matemática Computacional II \\ 
...
\textbf{General:} Formular, analizar, simular e interpretar modelos matemáticos determinísticos---principalmente basados en ecuaciones diferenciales y sistemas dinámicos---a partir de problemas de ciencias, ingeniería o contextos interdisciplinarios, evaluando la validez del modelo y comunicando resultados con claridad.
...
\begin{enumerate}
    \item Principios de balance y conservación; construcción de EDO y sistemas autónomos en contextos físicos y biológicos.
    ...
    \item Bifurcaciones elementales y cambios cualitativos en la dinámica (ejemplos motivadores).
    \item Casos integradores: oscilaciones y péndulo (incluyendo registro experimental sencillo, p.\ ej.\ con sensores de un dispositivo móvil); modelos de reacción--difusión o competencia en versión reducida.
    \item Modelos clásicos en epidemiología (SIR y variantes) y otros ejemplos de dinámica poblacional o social.
\end{enumerate}

\item \textbf{Modelación discreta y simulación computacional (3 semanas):}
\begin{enumerate}
    \item Ecuaciones en diferencias y mapas iterados; relación con modelos continuos (Euler, discretización).
    \item Métodos numéricos para EDO: Euler, Runge--Kutta; error de truncamiento y estabilidad a nivel práctico.
    \item Exploración de parámetros, bifurcaciones numéricas y visualización de retratos de fase.
    \item Introducción opcional a modelos en redes (grafos) según criterio de la persona docente.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Estimación de error y complementos probabilísticos básicos para cuantificar incertidumbre (sin exigir un curso avanzado de inferencia).
    \item Comunicación de resultados a públicos especializados y no especializados.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item No se exige un software único; se aceptan entornos de computación científica (Python, Julia, R, Octave, \emph{SageMath}, etc.) según disponibilidad y criterio docente, priorizando reproducibilidad y documentación del código.
