\parindent=0pt
\parskip=5pt
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%% Information about using colors in LaTeX %%%%%%%%%%%%%%%%
%%%% https://www.overleaf.com/learn/latex/Using_colours_in_LaTeX %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage[svgnames, dvipsnames, table]{xcolor}
%\usepackage[colorlinks = true, allcolors = Cerulean, pagebackref=true, colorlinks,]{hyperref}
%\renewcommand\backrefxxx[3]{%
 % \hyperlink{page.#1}{$\uparrow$#1}%
%}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{placeins}
\usepackage[left=3cm,right=3cm,top=1cm,bottom=3cm]{geometry} 
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\renewcommand{\familydefault}{\sfdefault}
\usepackage[spanish, es-tabla]{babel}
\usepackage{fancyhdr}
\usepackage{afterpage}
\usepackage[colorlinks = true, allcolors = Cerulean]{hyperref}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{calc}
\usepackage{sectsty}
\usepackage[table]{xcolor}
\usepackage{longtable}
\usepackage{float}
\usepackage{rotating}
\usepackage{tablefootnote}
\usepackage{multicol}
\usepackage{multirow}
\usepackage{hyperref}
\usepackage{array}
\usepackage{rotfloat}
\usepackage{pdflscape}
\usepackage[backend=biber, style=alphabetic]{biblatex}
\providecommand{\BiblioFile}{Biblio.bib}
\addbibresource{\BiblioFile}
\newcommand{\printcursosbibliography}{%
  \printbibliography[heading=subbibliography,section=\the\value{refsection}]%
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%% Definición de colores UCR %%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\definecolor{Celeste-UCR}{RGB}{0, 192, 243}
\definecolor{Azul-UCR}{RGB}{0, 93, 164}
\definecolor{Verde-UCR}{RGB}{109, 192, 103}
\definecolor{Amarillo-UCR}{RGB}{255, 224, 106}

\sectionfont{\color{Azul-UCR}}
\subsectionfont{\fontsize{13.3}{13}\selectfont \color{Azul-UCR}}
\subsubsectionfont{\fontsize{13.3}{13}\selectfont \color{Azul-UCR}}

\newcommand{\N}{\mathbb{N}}
\newcommand{\Z}{\mathbb{Z}}
\newcommand{\C}{\mathbb{C}}
\newcommand{\Q}{\mathbb{Q}}
\newcommand{\I}{\mathbb{I}}
\newcommand{\R}{\mathbb{R}}
\newcommand{\op}[1]{\operatorname{#1}}
\newcommand{\bb}[1]{\mathbb{#1}}
\newcommand{\fr}[1]{\mathfrak{#1}}
\newcommand{\legendre}[2]{\left( \dfrac{#1}{#2} \right)}
\newcommand{\mcd}{\operatorname{mcd}}

% Programas individuales: no figuran en la tabla de contenidos del documento unificado.
\newcommand{\inputcurso}[1]{%
  \begingroup
  \renewcommand{\addcontentsline}[3]{}%
  \input{#1}%
  \endgroup
}

\input{definitions}

\title{Propuestas de modificación curricular\\ \vspace{4pt} \large Bachillerato y Licenciatura en Matemática\\ \vspace{10pt} Escuela de Matemática \\ \vspace{2pt}  Universidad de Costa Rica}
\author{Comisión de Estructura Curricular}
\date{\today}
